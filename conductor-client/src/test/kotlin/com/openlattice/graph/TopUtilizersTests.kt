...
class TopUtilizersTests {
    ...
    fun testNewTopUtilizers() {
        val sql = buildTopEntitiesQuery(false)
        logger.info(sql)
    }

    @Test
    fun testNewTopUtilizersLinking() {
        val sql = buildTopEntitiesQuery(true)
        logger.info(sql)
    }
    ...
    private fun buildTopEntitiesQuery(linking: Boolean): String {
        ...
        val dataSourceResolver = Mockito.mock(DataSourceResolver::class.java)
        val graph = Graph(dataSourceResolver, esService, partitionManager, pgDataQueryService, idService, metricRegistry)
