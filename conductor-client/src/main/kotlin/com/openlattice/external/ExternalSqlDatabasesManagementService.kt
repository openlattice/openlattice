...
class ExternalSqlDatabasesManagementService(
        ...
        return when (jdbcConnection.driver) {
            ...
            "com.snowflake.client.jdbc.SnowflakeDriver" -> SnowflakeExternalSqlDatabaseManager(jdbcConnection)
                    ...
                    "${jdbcConnection.driver}" +
                            " is not yet supported by OpenLattice platform. "
            )
        }
    }
    ...
    private fun fromMetadata( organizationId: UUID, tableMetadata: TableMetadata ) : ExternalSqlDatabaseTableMapping {
        ...
        tableMetadata.permissions.map{  ( dbUser, privileges ) ->
            dbCredentialService.lookupSecurablePrincipal( dbUser )
        }
    ...
    private fun fromMetadata( organizationId: UUID, columnMetadata: ColumnMetadata) :ExternalSqlDatabaseColumnMapping {
    ...
    fun createOrganizationExternalDatabaseTable(orgId: UUID, table: OrganizationExternalDatabaseTable): UUID {
        ...
        checkState(
                organizationExternalDatabaseTables.putIfAbsent(table.id, table) == null,
    ...
    fun createOrganizationExternalDatabaseColumn(orgId: UUID, column: OrganizationExternalDatabaseColumn): UUID {
        checkState(
                organizationExternalDatabaseTables[column.tableId] != null,
        ...
        checkState(
                organizationExternalDatabaseColumns.putIfAbsent(column.id, column) == null,
        ...
        authorizationManager.setSecurableObjectType(
                columnAclKey, SecurableObjectType.OrganizationExternalDatabaseColumn
        )
    ...
    fun getColumnMetadata(
        ...
        return BasePostgresIterable(
            ...
            try {
                ...
                logger.error(
                        "Unable to map column to OrganizationExternalDatabaseColumn object for table {} in organization {}",
                        tableName, orgId, e
                )
                null
            }
    ...
    fun transportEntitySet(organizationId: UUID, entitySetId: UUID) {
        val memberPrincipals = securePrincipalsManager.getOrganizationMembers(setOf(organizationId)).getValue(
                organizationId
        )
    ...
    fun getExternalDatabaseTablesWithColumns(
            orgId: UUID
    ): Map<Pair<UUID, OrganizationExternalDatabaseTable>, Set<Map.Entry<UUID, OrganizationExternalDatabaseColumn>>> {
    ...
    fun getExternalDatabaseTableWithColumns(tableId: UUID): OrganizationExternalDatabaseTableColumnsPair {
        ...
        return OrganizationExternalDatabaseTableColumnsPair(
                table, organizationExternalDatabaseColumns.values(
    ...
    fun updateOrganizationExternalDatabaseTable(orgId: UUID, tableFqnToId: Pair<String, UUID>, update: MetadataUpdate) {
        ...
        organizationExternalDatabaseTables.submitToKey(
                tableFqnToId.second, UpdateOrganizationExternalDatabaseTableEntryProcessor(update)
        )
    }

    fun updateOrganizationExternalDatabaseColumn(
            orgId: UUID, tableFqnToId: Pair<String, UUID>, columnFqnToId: Pair<String, UUID>, update: MetadataUpdate
    ) {
        update.name.ifPresent {
        ...
        organizationExternalDatabaseColumns.submitToKey(
                columnFqnToId.second, UpdateOrganizationExternalDatabaseColumnEntryProcessor(update)
        )
    }
    ...
    fun deleteOrganizationExternalDatabaseColumns(
            orgId: UUID, columnsByTable: Map<Pair<String, UUID>, Map<String, UUID>>
    ) {
    ...
    fun removeHBARecord(
            orgId: UUID, userPrincipal: Principal, connectionType: PostgresConnectionType, ipAddress: String
    ) {
        val dbName = externalDbManager.getOrganizationDatabaseName(orgId)
        val username = getDBUser(userPrincipal.id)
        val record = PostgresAuthenticationRecord(
    ...
    fun getOrganizationOwners(orgId: UUID): Set<SecurablePrincipal> {
        val principals = securePrincipalsManager.getAuthorizedPrincipalsOnSecurableObject(
                AclKey(orgId), EnumSet.of(Permission.OWNER)
        )
                ...
                columnAcls.forEach {
                    ...
                    it.aces.forEach { ace ->
                        ...
                        if (action == Action.SET) {
                            val revokeSql = createPrivilegesUpdateSql(
                                    Action.REMOVE, listOf("ALL"), tableName, columnName, dbUser
                            )
    ...
    private fun createPrivilegesUpdateSql(
            action: Action, privileges: List<String>, tableName: String, columnName: String, dbUser: String
    ): String {
        val privilegesAsString = privileges.joinToString(separator = ", ")
        checkState(
                action == Action.REMOVE || action == Action.ADD || action == Action.SET,
    ...
    private fun areValidPermissions(permissions: Set<Permission>): Boolean {
        if (!(permissions.contains(Permission.OWNER) || permissions.contains(Permission.READ) || permissions.contains(
                        Permission.WRITE
                ))) {
            return false
    ...
    private fun getPrivilegesFields(
            tableName: String, maybeColumnName: Optional<String>
    ): Pair<String, SecurableObjectType> {
        var columnCondition = ""
    ...
    private fun updateHBARecords(dbName: String) {
        val originalHBAPath = Paths.get(
                organizationExternalDatabaseConfiguration.path + organizationExternalDatabaseConfiguration.fileName
        )
        ...
        try {
            val out = BufferedOutputStream(
                    Files.newOutputStream(
                            tempHBAPath, StandardOpenOption.WRITE, StandardOpenOption.CREATE,
    ...
    private fun getCurrentUsersPrivilegesSql(
            tableName: String, grantsTableName: String, columnCondition: String
    ): String {
    ...
    private fun getInsertRecordSql(
            table: PostgresTableDefinition, columns: String, pkey: String, record: PostgresAuthenticationRecord
    ): String {
