...
class SearchLinkedEntitiesTests : SetupTestData() {
    companion object {
        ...
        fun init() {
            importedEntitySets.forEach {
                importDataSetFromCSV(it.value.first, it.value.second, true)
            }
    ...
    fun testDeleteLinkedEntitySet() {
        ...
        //redo import
        importDataSetFromCSV(importedEntitySets.values.last().first, importedEntitySets.values.last().second, true)
        Thread.sleep(10000L)
        while (!checkLinkingFinished(importedEntitySets.keys)) {
            Thread.sleep(5000L)
        }
