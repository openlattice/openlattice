...
public class DataController implements DataApi, AuthorizingComponent {
    ...
    public Integer replaceEntities(
        ...
        if (partialReplace) {
            ...
            return dgm.replaceEntities(entitySetId, transformValues(entities,Multimaps::asMap), dms.getPropertyTypesAsMap(requiredPropertyTypes));
        }
    }
    ...
    private Map<FullQualifiedName, Set<Object>> readFromJson(String json) {
        try {
            return ObjectMappers.getJsonMapper().readValue(json, new TypeReference<Map<FullQualifiedName, Set<Object>>>() {
            });
