...
class PostgresEntityKeyIdService(
    ...
    private fun storeEntityKeyIds(entityKeyIds: Map<EntityKey, UUID>): Map<EntityKey, UUID> {
        ...
        connection.use {
            ...
            entityKeyIds.forEach {
                ...
                ps.setObject(2, it.key.entitySetId)
                ps.setString(3, it.key.entityId)
                ps.addBatch()
            }
    ...
    private fun loadEntityKeys(entityKeyIds: Set<UUID>): Map<UUID, EntityKey> {
        val connection = hds.getConnection()
